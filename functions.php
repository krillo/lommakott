...
if (!function_exists('twentyeleven_comment')) :
  ...
  /**
   * Krillo - custom posttype for Lommakött 
   */
  add_action('init', 'register_cpt_produkt');

  function register_cpt_produkt() {

    $labels = array(
        'name' => _x('produkter', 'produkt'),
        'singular_name' => _x('produkt', 'produkt'),
        'add_new' => _x('Lägg till ', 'produkt'),
        'add_new_item' => _x('Lägg till ny produkt', 'produkt'),
        'edit_item' => _x('Redigera produkt', 'produkt'),
        'new_item' => _x('Ny produkt', 'produkt'),
        'view_item' => _x('Visa produkt', 'produkt'),
        'search_items' => _x('Sök produkter', 'produkt'),
        'not_found' => _x('Inga produkter funna', 'produkt'),
        'not_found_in_trash' => _x('Inga produkter funna i papperskorgen', 'produkt'),
        'parent_item_colon' => _x('Produkt förälder:', 'produkt'),
        'menu_name' => _x('Produkter', 'produkt'),
    );
  ...
  function init_order_table() {
    $version = 6;
    global $wpdb;
    $installed_ver = get_option("lommakottDbVersion");
    if ($installed_ver != $version) {
      require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
      $table_name = $wpdb->prefix . 'order_krillo';
      $sql = "CREATE TABLE `wp_order` (
              ...
              `orders` varchar(512) NOT NULL DEFAULT '',
              `sum` int(11) NOT NULL,
